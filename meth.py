...
def filtro(messagefilted):
            try:    
                regex = r"""([<@&|<#]+[0-9]+|[>])|[<#].[0-9]+[>]|[<@!]+[0-9]+[>]|([:|<|]+[a-z|a-z_]+[:]+[0-9]+)|([<|@!]+[0-9]+)|(POSTADO!!!)|(<https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z0-9]\.[^\s^\>]{2,})>|([<>])"""
                rege1 = r"""([<@&]+[0-9]+|[>])"""
                rege2 = r"""([<#]+[0-9]+|[>])"""
                ...
                if re.finditer(regex, messagefilted, re.MULTILINE | re.IGNORECASE | re.VERBOSE | re.DOTALL | re.UNICODE):
                    ...
                    messagefilted = re.sub(rege6, subst1, messagefilted, 0, re.MULTILINE | re.IGNORECASE | re.VERBOSE | re.DOTALL | re.UNICODE)
